...
mode = 'calibradte' # anything else is exploratory mode
    ...
    if mode == 'calibrate':
        ...
        with open('calib_nohands.csv', mode='r') as csvfile:
            ...
            for row in csvreader:  # Iterate through each row in the CSV file
                for iter in range(REPEAT):
                    ...
                    distance = linalg.norm(diff)
                    p.addUserDebugText(f"Calibrating {index+1}.pose {iter+1}.iteration", [.0, -0.3, .65], textSize=2, lifeTime=4, textColorRGB=[1, 0, 0])
                    p.addUserDebugText(f"X,Y,Z error: {diff}", [.0, -0.316, .60], textSize=2, lifeTime=4, textColorRGB=[1, 0, 0])
                    p.addUserDebugText(f"distance error: {distance}", [.0, -0.332, .55], textSize=2, lifeTime=4, textColorRGB=[1, 0, 0])
        ...
        while True:
            ...
            if ord('q') in keypress:
                break
            all_position = get_real_joints(robot, init_pos.keys())
            p.addUserDebugText(f"Pos: {all_position}", [.0, -0.316, .60], textSize=2, lifeTime=0.05, textColorRGB=[1, 0, 0])
            keypress.clear()
            spin_simulation(1)
