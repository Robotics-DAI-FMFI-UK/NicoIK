...
def main():
    ...
    box_id = p.createMultiBody(
        ...
        basePosition=[0.35, 0, 0.05]  # Initial position
    )
    
    # Create sliders for box position control
    x_slider = p.addUserDebugParameter("Box X", 0.2, 0.5, 0.35)
    y_slider = p.addUserDebugParameter("Box Y", -0.3, 0.3, 0)
    z_slider = p.addUserDebugParameter("Box Z", 0.05, 0.5,0.05)
    ...
    try:
        while True:
            ...
            if ord('r') in keys and keys[ord('r')] & p.KEY_WAS_TRIGGERED:
                ...
                ee_pos = [0.3, 0.0, 0.2]  # Fixed position for testing
                # Calculate IK with fixed orientation
                ...
                for joint_idx, slider_id in sliders:
                    joint_pos = ik_solution[joint_idx]
